...
class Merchant(NPC):
    ...
    def open_shop(self):
        ...
        while self.flag:
            ...
            self.money_left.blit()  # by Taisia
            clock.tick(FPS)
        ...
        try:
            for txt_box, line in zip(self.inserters, self.shop):
                ...
                if amount < 0:
                    raise ValueError
                if amount > 0:
                    if int(line[-1] * amount) < self.result:
                        purchase.append(line + [amount])
                        total += int(line[-1] * amount)
                    else:
                        not_enough = thorpy.make_text("Недостаточно денег!",
                                                      font_size=15, font_color=(0, 0, 0))
                        not_enough.set_topleft((150, 5))
                        not_enough.blit()
                        not_enough.update()
                        time.sleep(1)  # окно открыто секунду, потом закрывается
            # global money -> убрала
            if total > self.result:  # поменяла money на self.result, потому что у меня уже объявлено количество денег
                raise ValueError
            # money -= total  -> убрала
            self.result -= total
            self.cur_taisia_db.execute('UPDATE users SET money=? WHERE name="111"', (self.result, ))
